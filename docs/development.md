...
Working on the site: the stack, the layout of the tree, i18n, and placeholder
mode.
...
src/
  ...
  data/chains.yaml      NXPC outside Henesys, sourced from an internal document
...
`base: '/nxpc-supply'` in `astro.config.mjs` is the project-page path. Renaming the
repository means changing it too.

## Placeholder mode

A registry can be marked `# placeholder` while its addresses are being filled
in; the page then shows a notice and uses stand-in balances. To leave that mode:
...
6. Build. The placeholder banners disappear and the snapshot archive starts
   recording.
