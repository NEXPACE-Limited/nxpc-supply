...
**→ [nexpace-limited.github.io/nxpc-supply](https://nexpace-limited.github.io/nxpc-supply/)**
...
| **Circulating supply** | What is left. This is the headline figure, and it follows NEXPACE's internal token circulating supply policy. |
...
The Henesys figures come from Henesys (`henesys-rpc.msu.io`) and nowhere else.
Avalanche C-Chain is read separately, through Ava Labs' public node, and it is
read for two things: the C-Chain balances the address tab shows, and Avalanche
circulating supply — `totalSupply − CCIP pool − the addresses we operate there`
— which free float subtracts. The addresses in that last term are the `c-chain`
rows of the address registry, so adding or removing one moves free float as well
as the table. If that endpoint is unreachable the Avalanche figure falls back to
the surveyed number in `chains.yaml` and the page still builds.
...
| [docs/development.md](docs/development.md) | Stack, repository layout, i18n, deployment, and placeholder mode |
